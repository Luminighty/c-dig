...
ColliderMoveResult collider_move(ColliderId id, Vec2 delta) {
	...
	result.resolved_position = slide_result.resolved_position;
...
void physics_render() {
	...
	for (int x = min.x; x < max.x; x++) {
		...
		Color unselected = ColorAlpha(RED, 0.1);
		Color selected = ColorAlpha(GREEN, 0.5);
		draw_box(
			tile_box.center.x - tile_box.extends.x,
			tile_box.center.y - tile_box.extends.y,
			tile_box.extends.x * 2,
			tile_box.extends.y * 2,
			world_mouse_tile.x == x && world_mouse_tile.y == y ? selected : unselected
		);
	}}
	if (input.clicked) {
